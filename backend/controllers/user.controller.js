...
exports.signup = (req, res, next) => {
    sql.query(`SELECT * FROM user WHERE email = '${req.body.email}'`, /*async*/ (error, result) => {
        if(error) {console.log('erreur sql 1'); return res.status(500).json({ error });}
        console.log('result sql 1', result);
        if(result[0]) {
            console.log('erreur email utilisée');
            return res.status(400).json({ message: 'The email is already used'});
        } else {
            console.log('result empty array');
            console.log('req.bodypassword before sql2', req.body.password);
            ...
            .then(hash => {
                ...
                if(error) {console.log('erreur sql 2'); return res.status(400).json({ error })}
                return res.status(201).json({message: 'User created!'})
                })
            })
            .catch(error => {console.log('erreur bcrypt'); return res.status(500).json({ error })})
        }
    })
...
exports.login = (req, res, next) => {
    console.log('login');
    sql.query(`SELECT * FROM user WHERE email = '${req.body.email}'`, /*async*/ (error, result) => {
        console.log('login sql1')
        if(error) {console.log('error sql1login'); return res.status(500).json({ error });}
        else {
            console.log('login sql1 no error')
            ...
            .catch(error => {
                console.log('erreur bcryptcompare');
